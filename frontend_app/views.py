...
class MainPageView(View):
    ...
    def get(self, request):
        req = requests.get('https://pokeapi.co/api/v2/pokemon-species/?limit=100')

        # _____delete after tests_____ #
        filename = "tests/test.json"
        with open(filename, encoding="utf-8") as fh:
            concerts = json.loads(fh.read())
        # _____delete after tests_____ #

        if req.status_code == requests.codes.ok:
            # data = req.json() # return

            if len(concerts) > 12:
                concerts = concerts[:12]

            concerts = sorted(concerts, key=lambda x:
                datetime.strptime(x["event_date"] + " " + x["event_time"], "%Y-%m-%d %H-%M"))
